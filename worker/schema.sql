...
CREATE TABLE IF NOT EXISTS service_categories (
  ...
  -- of {id, heading, items:[{id, text}]}, editable from Settings → Service
  -- Lines. When a proposal adds this service on Step 2, Step 3 pre-fills
...
-- One row per operating region (au/uk/ie). Feeds Hotel Details (Step 1) of
-- the proposal wizard: selecting a region pre-fills the Nuvho entity address,
-- legal footer, currency, and default Terms & Conditions clauses for that
-- jurisdiction. Editable from Settings → Region Settings. `clauses_json`
-- follows the same shape/pattern as proposal_terms.clauses_json — an ordered
-- list of {id, heading, text, enabled} with no independent relational identity.
