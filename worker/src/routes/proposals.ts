...
export async function getProposal(proposalId: string, env: Env, session: Session): Promise<Response> {
  ...
  // Nest each service's scope items, fee rows, and footnotes (Scope/Pricing
  // wizard steps) back onto its row, translated into the camelCase shape the
  // frontend wizard expects (see lib/types.ts DraftServiceLine).
  const servicesWithChildren = await Promise.all(services.map(async (svc) => {
